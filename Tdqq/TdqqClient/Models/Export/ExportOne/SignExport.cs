    ...
    class SignExport:ExportOne
    {
        ...
        private void ExportF(object p)
        {
            ...
            try
            {
                ...
                IDatabaseService accessFactory = new MsAccessDatabase(PersonDatabase);
        ...
        private int FillFieldSign(System.Data.DataTable dtDkSingle, HSSFWorkbook workbookSource, int endRow, ref double htmj, ref double scmj)
        {
            ...
            for (int j = 0; j < dtDkSingle.Rows.Count; j++)
            {
                ...
                //四至
                rowSource.GetCell(8).SetCellValue(EditSz(dtDkSingle.Rows[j][3].ToString()));
                rowSource.GetCell(9).SetCellValue(EditSz(dtDkSingle.Rows[j][4].ToString()));
                rowSource.GetCell(10).SetCellValue(EditSz(dtDkSingle.Rows[j][5].ToString()));
                rowSource.GetCell(11).SetCellValue(EditSz(dtDkSingle.Rows[j][6].ToString()));
                //合同面积和实测面积
