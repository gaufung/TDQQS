...
namespace TdqqClient.Models.Export.ExportOne
{
    ...
    class PostExport:ExportOne
    {
        ...
        {           
            return SelectCbfbmOwnFields().Rows.Count;
        }
        /// <summary>
        /// 获取宗地数目
        /// </summary>
        ...
        {
            var sqlString = string.Format("select Count(*) from {0} where CBFBM NOT LIKE '{1}' ",
        ...
        {
            var pAccess = new MsAccessDatabase(PersonDatabase);
            string sqlString = string.Format("select SUM(SCMJ) from {0}", SelectFeature);
            double scmj;
            var ret = double.TryParse(pAccess.ExecuteScalar(sqlString).ToString().Trim(), out scmj);
            if (ret)
            {
                return scmj;
            }
