...
namespace TdqqClient.Models.Export.ExportOne
{
    class DExport:ExportOne
    {
        ...
        private void Export(HSSFWorkbook workbook, System.Data.DataRow row, ref int startRowIndex, ICellStyle style, int index)
        {
            ...
            var cbdks = SelectFieldsByCbfbm(cbfbm);
            double htmjSum = 0.0;
            double scmjSum = 0.0;
            var cbfmc = row[1].ToString().Trim();
            var sheet = workbook.GetSheetAt(0);
            for (int j = 0; j < cbdks.Rows.Count; j++)
            {

                var rowSource = (HSSFRow)sheet.GetRow(startRowIndex);
                if (cbdks.Rows.Count == 1) rowSource.Height = 1200;                
                //DKMC
                rowSource.GetCell(4).SetCellValue(cbdks.Rows[j][1].ToString());
                //DKBM
                var dkbm = cbdks.Rows[j][3].ToString().Trim();
                if (dkbm != string.Empty) dkbm = dkbm.Substring(14, 5);
                rowSource.GetCell(5).SetCellValue(dkbm);
                //四至
                rowSource.GetCell(6).SetCellValue(EditSz(cbdks.Rows[j][4].ToString()));
                rowSource.GetCell(7).SetCellValue(EditSz(cbdks.Rows[j][5].ToString()));
                rowSource.GetCell(8).SetCellValue(EditSz(cbdks.Rows[j][6].ToString()));
                rowSource.GetCell(9).SetCellValue(EditSz(cbdks.Rows[j][7].ToString()));
                //合同面积
                double htmjSingle=string.IsNullOrEmpty(cbdks.Rows[j][16].ToString())?
                    0.0 : double.Parse(cbdks.Rows[j][16].ToString().Trim());
                htmjSum += htmjSingle;

                double scmjSingle=string.IsNullOrEmpty(cbdks.Rows[j][17].ToString().Trim())?
                    0.0 : double.Parse(cbdks.Rows[j][17].ToString().Trim());               
                scmjSum += scmjSingle;
